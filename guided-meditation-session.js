...
function startMeditationJourney() {
    ...
    // Add memory color parameter if selected
    if (selectedMemoryColor) {
        params.append('memoryColor', selectedMemoryColor);
    }
...
function endSession() {
    ...
    // Stop memory color music if playing
    if (memoryAudio) {
        memoryAudio.pause();
        memoryAudio = null;
    }
...
function resetSession() {
    ...
    selectedSecondaryMoods = [];
    selectedScene = null;
    selectedMemoryColor = null;
    
    // Reset UI
    document.querySelectorAll('.primary-mood-btn, .mood-btn, .scene-btn, .memory-color-btn').forEach(btn => {
        btn.classList.remove('selected');
    });
    ...
    // Hide sections
    document.getElementById('secondaryEmotions').classList.add('hidden');
...
// Function to handle memory color selection
function selectMemoryColor(color) {
    ...
    playMemoryMusic(color);
    
    // Show toast notification
    showToast(`Selected ${getColorName(color)} memory color`);
}

// Function to play music based on selected color
function playMemoryMusic(color) {
    // 本地音频文件映射
    const localMusicMap = {
        'blue': './audio/meditation-relaxing-music.mp3',
        'green': './audio/meditation-flute-music.mp3',
        'purple': './audio/meditation-deep-music.mp3',
        'yellow': './audio/meditation-immersive-music.mp3',
        'pink': './audio/meditation-relaxing-music.mp3',  // 暂时使用相同的音乐
        'orange': './audio/meditation-flute-music.mp3'    // 暂时使用相同的音乐
    };
    
    // 远程音频文件映射（备用）
    const remoteMusicMap = {
        'blue': './audio/meditation-relaxing-music.mp3',
        'green': './audio/meditation-flute-music.mp3',
        'purple': './audio/meditation-deep-music.mp3',
        'yellow': './audio/meditation-immersive-music.mp3',
        'pink': './audio/meditation-relaxing-music.mp3',
        'orange': './audio/meditation-flute-music.mp3'
    };
    
    // 尝试加载本地音频文件
    memoryAudio = new Audio(localMusicMap[color]);
    memoryAudio.volume = 0.5;
    memoryAudio.loop = true;
    
    // 监听错误事件，如果本地文件加载失败，则尝试加载远程文件
    memoryAudio.addEventListener('error', function() {
        console.log('Local audio loading failed, trying backup audio');
        memoryAudio = new Audio(remoteMusicMap[color]);
        memoryAudio.volume = 0.5;
        memoryAudio.loop = true;
        memoryAudio.play().catch(error => {
            console.error('Backup audio loading failed:', error);
            showToast('Unable to play music, please ensure audio files are available');
        });
    });
    ...
    memoryAudio.play().catch(error => {
        ...
        showToast('Unable to play music, please click on the page and try again');
    });
}

// Helper function to get color name in English
