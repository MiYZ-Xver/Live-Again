...
function getUrlParams() {
    ...
    return {
        ...
        memoryColor: params.get('memoryColor')
    };
}
...
function initMeditationJourney() {
    ...
    // Apply memory color if provided
    if (params.memoryColor) {
        selectedMemoryColor = params.memoryColor;
        applyMemoryColor(params.memoryColor);
        playMemoryMusic(params.memoryColor);
    }
...
function togglePlayPause() {
    ...
    if (isPaused) {
        ...
        // Resume memory audio if available
        if (memoryAudio) {
            memoryAudio.play().catch(error => {
                console.error('Failed to resume audio:', error);
                showToast('Unable to resume music, please click on the page and try again');
            });
        }
...
function restartSession() {
    ...
    // Restart memory music if needed
    if (selectedMemoryColor && !memoryAudio) {
        playMemoryMusic(selectedMemoryColor);
    }
...
// Apply memory color to UI elements
function applyMemoryColor(color) {
    const colorMap = {
        'blue': '#A0C4FF',
        'green': '#CAFFBF',
        'purple': '#BDB2FF',
    ...
    const selectedColor = colorMap[color] || '#B7C4B7';
    
    // Update progress circle color
    const progressCircle = document.getElementById('progressCircle');
    if (progressCircle) {
        progressCircle.setAttribute('stroke', selectedColor);
    }
    
    // Update breathe circle color
    const breatheCircle = document.querySelector('.breathe-circle');
    if (breatheCircle) {
        breatheCircle.style.backgroundColor = `${selectedColor}80`; // 50% opacity
    }
    
    // Update play/pause button color
    const playPauseBtn = document.getElementById('playPauseBtn');
    if (playPauseBtn) {
        playPauseBtn.style.backgroundColor = selectedColor;
    }
}

// Play music based on selected memory color
function playMemoryMusic(color) {
    ...
    if (volumeControl) {
        memoryAudio.volume = volumeControl.value / 100;
        volumeControl.addEventListener('input', function() {
            if (memoryAudio) {
                memoryAudio.volume = this.value / 100;
            }
        });
    }
